...
* **粘贴图片**：在预览或 Markdown 模式下粘贴剪贴板图片时，支持设置保存路径/文件名与插入方式（引用文件或 Base64）；默认保存到当前 .md 同目录 `images/`，默认命名为 `image-YYYYMMDD-NNN.png`。预览中图片悬停时支持在「引用文件」与「Base64 内联」之间双向切换。

* **在页面树中定位**：平铺模式下对某个 .md 文件右键，在「在 Finder 中打开」下方新增「在页面树中定位」；点击后切换到树状模式并自动展开该文件所属文件夹路径、滚动到该节点并选中。

### 优化

* **预览复制与顶部栏样式**：复制按钮移至预览区域顶部条左侧；复制按钮、预览/Markdown 选项卡采用浅灰样式弱化，激活态为略深灰而非蓝色。

* **页面树 getParent 缓存**：树状模式下对节点做路径缓存，使 `reveal` 能正确展开所属文件夹路径。

* **新建笔记/文件夹**：仅当有文件夹被激活（选中或右键在文件夹上）时在该文件夹下新建，否则在根目录新建。

### 修复

* **拖拽移出文件夹**：修复从子文件夹拖到树空白/根目录时误报「移动失败」的问题。

## \[0.2.4] - 2026-03-04

### 优化

* **预览中表格列宽**：优化预览中表格的列宽显示逻辑，支持智能识别内容长度并自动调整列宽；优化了显示的字体大小。

## 0.2.3] - 2026-03-03

### 新增

* **编辑器撤销/重做**：Cora 预览编辑器支持 Cmd+Z 撤销、Cmd+Shift+Z 重做，覆盖 Visual 模式、Markdown 源码模式和 Marked 只读模式；右上角「更多」菜单中可使用撤销和重做。

### 修复

* **双重显示问题**：修复从资源管理器、聊天链接等非 Cora 入口打开 .md 时被强制替换为 Cora 预览的问题。现在仅在 Cora 内部入口（页面树、命令等）打开时才使用 Cora 预览，其它入口保持系统原生行为。

## 0.2.2] - 2026-03-02

### 优化

* **CoraWiki 报告稳定性优化**：提高 FINAL 输出解析鲁棒性，显著降低异常输出导致的报告字段缺失，确保关键章节稳定生成。

## 0.2.1] - 2026-03-02

### 优化

* **CoraWiki 报告质量**：优化了分析策略与提示词约束，显著提升架构洞察深度、准确性与报告可读性。
