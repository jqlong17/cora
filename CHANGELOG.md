...
* **粘贴图片**：在预览或 Markdown 模式下粘贴剪贴板图片时，弹出自定义弹窗一次性填写保存路径/文件名与插入方式（引用文件或 Base64）；默认保存到当前 .md 同目录 `images/`，默认命名为 `image-YYYYMMDD-NNN.png`；无论选哪种插入方式，文件都会保存到 `images/` 作为备用。预览中图片悬停时右上角可切换「引用文件」/「Base64 内联」。
