...
| **规划** | Plan 书写约束（实验功能，可选） | 写 Plan 时有结构、可验收；AI 写 plan 时先读约束，输出更一致。 |
...
**Plan 增强**为实验功能。Cora 提供 **Plan 书写约束说明与模板**，帮助人与 AI 写出结构清晰、可验收的 Plan。在 Cursor 中可配合工作区内的约束与规则，实现「写 plan 时先读约束」，使 AI 输出的 Plan 更一致、可执行。后续版本可能提供「打开约束说明」命令或「从模板新建 Plan」等入口。
...
* **Plan 增强**（实验功能）：Plan 书写约束文档与模板作为产品能力，后续可能提供「打开约束说明」命令或「从模板新建 Plan」入口。
