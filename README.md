...
* **三种视图模式**: 工具栏提供独立按钮切换——**树状**（默认，按文件夹层级）、**平铺**（列表按修改时间降序）、**收藏**（仅显示已收藏的 Markdown 文件）

* **收藏功能**: 在树状/平铺下，鼠标悬停或右键 MD 文件可「收藏」/「取消收藏」；已收藏文件显示星标，收藏列表在「收藏」视图中查看

* **工具栏顺序**: 树状 → 平铺 → 收藏 → 排序 → 筛选 → 刷新 → 新建文件 → 新建文件夹
