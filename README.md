...
### 📄 页面树 (Page Tree)
- **平铺/树形切换**: 默认以平铺列表展示所有 Markdown 文件，按修改时间降序；可切换为按文件夹层级的树形视图
...
- **快速跳转**: 点击大纲项跳转到对应位置（预览模式下自动切换到编辑模式）
...
2. **浏览文件（页面树）**
   - 在【页面】标签中查看工作区文件树；默认**平铺**展示所有 Markdown，按修改时间降序
   - 点击标题栏「切换平铺/树形」在平铺与树形之间切换；树形下可使用【全部/Markdown】过滤
   - 右键文件可进行各种操作
...
| `knowledgeBase.pageViewMode` | string | `"flat"` | 页面展示模式：`flat` 平铺按修改时间排序，`tree` 树形层级 |
